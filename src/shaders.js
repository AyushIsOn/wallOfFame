...
export const tileFragmentShader = `
  ...
  void main() {
    ...
    // Dominant-color hover glow: a flat, darkened wash filling the whole cell
    // (no soft/rounded edge; the grid line defines the boundary).
    vec3 color = mix(uAvg * (uHover * 0.55), c.rgb, c.a);

    // Crisp grid lines (~1.5px at any cell size) via screen-space derivatives,
    // computed per-axis so there is no diagonal artifact.
    ...
    color = mix(color, uGrid.rgb, max(lx, ly) * uGrid.a);
